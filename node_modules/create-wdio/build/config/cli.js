...
function buildTauriBanner(driverProvider, useFrontendPlugin) {
  const lines = [
    "",
    "\u{1F980} Tauri requires a couple of Rust-side additions before tests can run:",
    ""
  ];
  if (driverProvider === "Embedded driver via tauri-plugin-wdio-webdriver crate" /* Embedded */) {
    lines.push(
  ...
  } else if (driverProvider === "Official tauri-driver (cross-platform, Cargo-installed)" /* Official */) {
    lines.push(
  ...
  } else if (driverProvider === "CrabNebula tauri-driver (managed cloud / drop-in)" /* CrabNebula */) {
    lines.push(
      "  1. Follow the CrabNebula tauri-driver setup guide:",
      "       \u{1F517} https://crabnebula.dev/docs/tauri-driver",
      ""
    );
  }
  ...
  lines.push(
    ...
    "  \u{1F517} https://webdriver.io/docs/desktop-testing/tauri"
  );
  return lines.join("\n");
}
...
var DesktopFrameworkChoice = /* @__PURE__ */ ((DesktopFrameworkChoice2) => {
  DesktopFrameworkChoice2["Electron"] = "Electron (https://www.electronjs.org/)";
  DesktopFrameworkChoice2["Tauri"] = "Tauri (https://tauri.app/)";
  DesktopFrameworkChoice2["MacOS"] = "Native macOS app";
  return DesktopFrameworkChoice2;
})(DesktopFrameworkChoice || {});
var TauriDriverProviderChoice = /* @__PURE__ */ ((TauriDriverProviderChoice2) => {
  TauriDriverProviderChoice2["Official"] = "Official tauri-driver (cross-platform, Cargo-installed)";
  TauriDriverProviderChoice2["CrabNebula"] = "CrabNebula tauri-driver (managed cloud / drop-in)";
  TauriDriverProviderChoice2["Embedded"] = "Embedded driver via tauri-plugin-wdio-webdriver crate";
  return TauriDriverProviderChoice2;
...
function getResolvedPurpose(answers) {
  const raw = getTestingPurpose(answers);
  if (raw !== "desktop") {
    return raw;
  }
  switch (answers.desktopFramework) {
    case "Tauri (https://tauri.app/)" /* Tauri */:
      return "tauri";
    case "Native macOS app" /* MacOS */:
      return "macos";
    case "Electron (https://www.electronjs.org/)" /* Electron */:
    default:
      return "electron";
  }
}
...
var QUESTIONNAIRE = [{
  ...
  when: (
    /* istanbul ignore next */
    (answers) => getTestingPurpose(answers) === "desktop"
  )
}, {
  type: "list",
  ...
  when: (
    ...
    (answers) => getResolvedPurpose(answers) === "electron"
  )
}, {
  type: "input",
  ...
  when: (
    /* istanbul ignore next */
    (answers) => getResolvedPurpose(answers) === "electron" && answers.electronBuildTool === "Something else" /* SomethingElse */
  )
}, {
  type: "list",
  ...
  when: (
    /* istanbul ignore next */
    (answers) => getResolvedPurpose(answers) === "tauri"
  )
}, {
  type: "confirm",
